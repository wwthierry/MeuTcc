// Gerar um ID único para o usuário (simula autenticação)
...
async function loadPosts() {
  try {
    ...
    if (posts.length === 0) {
      postsContainer.innerHTML = '<div class="empty-state">Nenhuma publicação ainda. Seja o primeiro a compartilhar! 📷</div>';
      return;
    }
    ...
    postsContainer.innerHTML = '<div class="empty-state">Erro ao carregar publicações 😞</div>';
  }
}
...
async function createPostElement(post) {
  ...
  postDiv.innerHTML = `
    <div class="post-header">
      ...
      <div class="post-meta">Por ${escapeHtml(post.author)} • ${postDate}</div>
    </div>
    ...
    <div class="post-actions">
      <button class="action-btn ${likeStatus.liked ? 'liked' : ''}" onclick="toggleLike(${post.id}, this)">
        ${likeStatus.liked ? '❤️' : '🤍'} <span class="like-count">${likeStatus.likesCount}</span>
      </button>
    ...
    <div class="comments-section" id="comments-${post.id}" style="display: none;">
      <div class="comments-list" id="comments-list-${post.id}">
        ${comments.map(comment => `
          <div class="comment">
            <div class="comment-header">${escapeHtml(comment.author)} • ${new Date(comment.createdAt).toLocaleDateString('pt-BR')}</div>
...
window.addComment = async function(event, postId) {
  ...
  try {
    const response = await fetch(`/api/posts/${postId}/comments`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ text, author: 'Anônimo' })
    });
...
document.querySelectorAll('.post-btn').forEach(btn =>{
    btn.addEventListener('click', () => {
      window.location.href = 'public/publi.html';
    });
});